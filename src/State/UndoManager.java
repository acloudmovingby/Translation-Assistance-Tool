...
public class UndoManager {
    ...
    /**
     * A stack representing the prior states of the program. When undo is performed, the top of the stack is popped and added to the redoStack.
     */
    private final Deque<MainFile> undoStack;
    
    /**
     * A stack representing "future" MainFile states that have been undone but could be redone (redo-ed ?).
     */
    ...
    /**
     * Stores the current state (currently just the MainFile), so that if undo is called the state can be restored to this version.
     * 
     * Note: THIS CLEARS REDOSTACK. UndoManager does not support branching file history, so if undo is performed and then a new state is pushed onto the undo stack, the redo is cleared (so no redo can then be performed).
     * 
     * @param state 
     */
    ...
    /**
     * MUTATES STATE. This replaces the MainFile object stored in State with the specified MainFile.
     * @param newFile
     * @param state 
     */
